...
**Updated:** 2026-08-21 (#200 merged 10:25 UTC — this section listed it as Open for the THIRD
time; a PR's state is a fact to query, not to remember)
...
- **PR (draft) #203 `claude/software-hardware-dev-analogy-8xu0uw`** — docs only:
  `notes/organism-izz.md`, the system described as an organism, with a measured inventory and
  proposals (none implemented). Registry gains `IZZ-0237`. Two findings to triage: `metrics.csv`
  stopped 2026-07-25 (so §19's "5.6x" rests on 3 July slices), and 14105 of 14653 tracked files
  are `media/` (265 MB on every checkout). `GEMINI_MODEL` is the one unpinned dependency.
