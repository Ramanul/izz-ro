...
def sample_meteo_sibiu() -> Image.Image:
    ...
    for i, (tM, tm, day) in enumerate(zip(tmax, tmin, days)):
        ...
        d.text((cx, min(ym + 8, y1 - 26)), f"{round(tm)}°", font=font(17), fill="#4a4d55", anchor="ma")
...
def sample_bilanț_nepal() -> Image.Image:
    ...
    x0, x1, y0, y1 = 64, 620, 200, 400
    pts = []
    for i, (_, v) in enumerate(serie):
        pts.append((x0 + i * (x1 - x0) / 3, y1 - v / 900 * (y1 - y0)))
    ov = Image.new("RGBA", img.size, (0, 0, 0, 0))
    ImageDraw.Draw(ov).polygon(pts + [(x1, y1), (x0, y1)], fill=(201, 162, 39, 52))
    ...
    for (x, y), (lbl, v) in zip(pts, serie):
        ...
        d.text((x, y1 + 14), lbl, font=font(15), fill="#c9ced8", anchor="ma")
    d.text((64, 436), "morți raportați în sintezele izz.ro", font=font(16), fill="#9aa0aa")
    d.text((660, 210), "≈800", font=font(92), fill=GOLD)
    d.text((664, 306), "morți", font=font(24), fill=PAPER)
    d.text((664, 348), "1500+ dispăruți", font=font(17), fill="#9aa0aa")
