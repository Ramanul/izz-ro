#!/usr/bin/env python
"""Scanner de verificare a site-urilor primăriilor din România.

  python tools/check_primarii.py --sample 40 --out data/primarii_status_sample.csv
  python tools/check_primarii.py --all --out data/primarii_status.csv

Auditează fiecare site din data/raport_complet_primarii.csv: accesibilitate,
verificare dacă e un site real de primărie, detecție CMS, existență RSS feed,
...
def detect_primarie(html: str, localitate: str) -> str:
    """Heuristici pentru a detecta dacă e un site de primărie."""
    if not html:
        return "unclear"
    html_lower = html.lower()
    keywords = ["primăria", "primaria", "consiliul local", "comuna", "oraș", "oraş", "municipiul", "municipiu"]
    localitate_lower = localitate.lower()
    found_primary = any(kw in html_lower for kw in keywords[:4])  # primăria/primaria/consiliul local/comuna
    found_secondary = any(kw in html_lower for kw in keywords[4:])  # oraș/municipiu
    found_locality = localitate_lower in html_lower if localitate else False
    if found_primary or (found_secondary and found_locality):
        return "yes"
    return "unclear"
...
def check_site(row: dict, timeout: int) -> dict:
    ...
    for try_url in tried_urls:
        ...
        if status and status < 400:
            ...
            result["is_primarie"] = detect_primarie(body, localitate)
...
def main():
    # Consola Windows poate fi cp1252 — diacriticele din nume de localități ar crăpa print-ul.
    import sys
    for stream in (sys.stdout, sys.stderr):
        if hasattr(stream, "reconfigure"):
            stream.reconfigure(encoding="utf-8", errors="replace")

    parser = argparse.ArgumentParser(description="Scanner de verificare a site-urilor primăriilor")
    parser.add_argument("--sample", type=int, help="Primele N rânduri din CSV")
    parser.add_argument("--all", action="store_true", help="Procesează toate rândurile")
    ...
    parser.add_argument("--out", default="data/primarii_status.csv", help="Path fișier output CSV")
    args = parser.parse_args()

    if not args.sample and not args.all:
        parser.error("Specifică --sample N sau --all")

    input_path = "data/raport_complet_primarii.csv"
    rows = []
    try:
    ...
    print(f"=== Check primării ({len(rows)} site-uri) ===")
    results = []
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                print(f"  {result['localitate']:20s} {result['http_status']:>4s} {result['is_primarie']:8s} {result['rss_ok']:3s} {result['error'] or ''}")
    ...
    out_path = args.out
    fieldnames = [
        "judet", "localitate", "url", "dns_ok", "http_status", "final_url",
        "https_ok", "is_primarie", "cms", "rss_url", "rss_ok",
    ...
    dead = total - alive
    real_primarie = sum(1 for r in results if r["is_primarie"] == "yes")
    ...
    print(f"Alive: {alive}")
    print(f"Dead: {dead}")
    print(f"Real primărie: {real_primarie}")
