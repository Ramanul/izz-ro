...
2. **Plan.** Name the exact files this slice will touch. Do not edit anything yet. If the slice is non-trivial, or touches clustering (§7 — must be verified empirically), or touches synthesis/attribution, anything legal/GDPR, or production deploy config (§10 — do not touch without explicit instruction), stop and let me confirm the plan before editing.
...
5. **Commit on green only.** Functional states are checkpoints: a verified slice gets committed before anything else starts, so there is always a working state to return to. One clear commit for the verified slice. Then stop and report the result — do not roll into the next slice.
