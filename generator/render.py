...
_CREDIT_REQUIRED_LICENSE = re.compile(r"^cc[ -]?by(?:[ -]sa)?[ -]\d", re.I)
...
def _load_leadphotos() -> dict:
    ...
    for key, rec in cache.items():
        if _leadphoto_is_publication_safe(rec):
            out[key] = {**rec, "credit_required": False}
        elif _leadphoto_is_article_only(rec):
            out[key] = {**rec, "credit_required": True}
...
def build(articles: list, mod: dict | None = None) -> None:
    ...
    for a in by_date:
        ...
        if lp and lp["credit_required"]:
            ...
            # `art_path`, deci scurgerea e imposibila prin constructie, nu prin vigilenta:
            # un template nou mostenit din `_card.html` nu vede niciodata `photo_path`.
            photo_dst = os.path.join(cdir, "photo.jpg")
            if _use_media(os.path.join(MEDIA_DIR, lp["art"]), photo_dst):
                a["photo_path"] = f"/{a['category']}/{a['slug']}/photo.jpg?v={_content_ver(photo_dst)}"
                photo_webp_dst = os.path.join(cdir, "photo.webp")
                if lp.get("webp") and _use_media(os.path.join(MEDIA_DIR, lp["webp"]), photo_webp_dst):
                    a["photo_webp"] = f"/{a['category']}/{a['slug']}/photo.webp?v={_content_ver(photo_webp_dst)}"
                a["lead_credit"] = lp   # obligatoriu aici: fara legenda, poza nu are voie sa apara
        elif lp:
