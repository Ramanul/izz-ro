...
def test_diversify_caps_fc_inter_1908_in_first_window():
    items = [_art("fcinter1908.it", i) for i in range(10)] + [
        _art("gsp.ro", i) for i in range(3)
    ] + [
        _art("digisport.ro", i) for i in range(2)
    ] + [
        _art("sport.ro", i) for i in range(2)
    ] + [
        _art("fanatik.ro", i) for i in range(3)
    ]
    ...
    assert first.count("fcinter1908.it") == 3
    assert len(first) == 10


def test_diversify_keeps_all_items_after_the_window():
    items = [_art("fcinter1908.it", i) for i in range(10)] + [_art("gsp.ro", 0)]
    out = render._diversify(list(items), max_run=2, max_per_window=3, window_size=10)
...
def test_diversify_does_not_cap_when_no_window_quota_is_requested():
    items = [_art("fcinter1908.it", i) for i in range(5)]
...
def test_diversify_uses_fallback_when_too_few_sources_exist():
    ...
    assert [a["source_name"] for a in out] == ["fcinter1908.it"] * 5
