...
- Tests: `python -m pytest tests/ -q` (**226 tests** as of 2026-08-02; CI runs them via `.github/workflows/tests.yml` on PRs + manual dispatch — deliberately NOT on push, because content commits land every 2h). The count is a rough marker, not a gate: check it against a real run before quoting it anywhere.
...
- **On variance:** home Perf was identical in 5 consecutive runs (80×5, article 88×5). One separate run under different machine load read home 84. That is a single observation, not a characterised variance — enough to say **re-run before calling a small delta a regression**, not enough to quote an error bar.
...
- **Why home Perf is 80, not the 89 recorded in July** (measured from the Lighthouse JSON, not guessed): CLS 0.224 scores 56 at weight 25, and FCP 2.6 s scores 65 at weight 10. The layout shift comes from `#izz-consent` plus web-font swap, and `styles.css` blocks render ~900 ms. TBT is a perfect 100. None of this is image-related — it is the open perf work, if you ever want it.
